...
function smartReframeCacheKey(opts: RenderOpts) {
  return createHash('sha256')
    .update(JSON.stringify({
      ...
      analysisFps: process.env.SMART_REFRAME_ANALYSIS_FPS || '12',
...
async function maybeBuildSmartCropExpressionUncached(opts: RenderOpts): Promise<SmartReframeResult> {
  ...
  try {
    ...
    let probe = await runJsonCommand(resolveSmartReframePython(), [
      ...
      String(opts.endSec),
      process.env.SMART_REFRAME_ANALYSIS_FPS || '12',
...
export async function renderVerticalClip(opts: RenderOpts) {
  ...
  const configuredPreset = (opts.fastRender ? process.env.FFMPEG_EDIT_X264_PRESET || 'veryfast' : process.env.FFMPEG_X264_PRESET || 'fast').trim();

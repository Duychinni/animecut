...
export function getClipPolicy(totalSeconds: number): ClipPolicy {
  ...
  if (minutes <= 7) {
    return { targetMin: 6, targetMax: 10, candidateCount: 40, expectedMinSec: 25, expectedMaxSec: 45, minSec: 20, maxSec: 90 };
  }
  if (minutes <= 10) {
    return { targetMin: 10, targetMax: 16, candidateCount: 72, expectedMinSec: 20, expectedMaxSec: 48, minSec: 18, maxSec: 90 };
  }
  if (minutes <= 20) {
